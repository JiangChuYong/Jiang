...
-(CGSize)boundingRectWithSize:(CGSize)size
    withTextFont:(UIFont *)font withLineSpacing:(CGFloat)lineSpacing;
...
- (CGSize)textSizeWithFont:(UIFont *)font constrainedToSize:(CGSize)size lineBreakMode:(NSLineBreakMode)lineBreakMode
{
    ...
    else
    {
		...
		{
			textSize = [self boundingRectWithSize:size withTextFont:font withLineSpacing:0];
		}
    }
    return textSize;
}
...
//        _leftTableView.separatorColor = [UIColor colorWithRed:220.f/255.0f green:220.f/255.0f blue:220.f/255.0f alpha:1.0];
		_leftTableView.separatorStyle = UITableViewCellSeparatorStyleNone;   // 这样就不会显示自带的分割线
...
//        _rightTableView.separatorColor = [UIColor colorWithRed:220.f/255.0f green:220.f/255.0f blue:220.f/255.0f alpha:1.0];
		_rightTableView.separatorStyle = UITableViewCellSeparatorStyleNone;   // 这样就不会显示自带的分割线
        ...
        _collectionView.backgroundColor = SpaceAlert_Gray_BGColor;
...
- (CAShapeLayer *)createIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point {
	...
	[bezierPath moveToPoint: CGPointMake(0, 7.5)];
	[bezierPath addLineToPoint: CGPointMake(6.62, 1)];
	[bezierPath addLineToPoint: CGPointMake(13.5, 7.5)];
	[bezierPath addLineToPoint: CGPointMake(10.72, 7.5)];
	[bezierPath addLineToPoint: CGPointMake(6.66, 3.45)];
	[bezierPath addLineToPoint: CGPointMake(2.72, 7.5)];
...
- (CGSize)calculateTitleSizeWithString:(NSString *)string
{
	...
	{
		 size = [string boundingRectWithSize:CGSizeMake(280, 0) withTextFont:[UIFont systemFontOfSize:fontSize] withLineSpacing:0];
	}
    return size;
}
...
- (void)animateIndicator:(CAShapeLayer *)indicator Forward:(BOOL)forward complete:(void(^)())complete {
	...
	{
		indicator.fillColor = self.highlightColor.CGColor;
	}
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
		...
		CGFloat tableViewHeight = 176;
		
		if(self.tableViewHeight != 0)
		{
			tableViewHeight = self.tableViewHeight;
		}
		else
		{
			tableViewHeight = MAX(leftTableViewHeight, rightTableViewHeight);
		}
...
- (void)animateTitle:(CATextLayer *)title show:(BOOL)show complete:(void(^)())complete {
	...
	if(show)
	{
		title.foregroundColor =  self.highlightColor.CGColor;
	}
	else
	{
		title.foregroundColor = self.indicatorColor.CGColor;
	}
    complete();
}
...
//            [cell addSubview:accessoryImageView];
        ...
        titleLabel.frame = CGRectMake(marginX, 0, textSize.width, cell.frame.size.height);
		cell.backgroundColor = SpaceAlert_Gray_BGColor;

		//没有选中的cell，且是左边的tableview
...
//                [cell addSubview:accessoryImageView];
			...
			else//没有选中的cell，且是左边的tableview，存在右边的tableview
			{
//				cell.backgroundColor = [UIColor redColor];

				UIImageView *backImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0, 8,45)];
				backImgView.image = [UIImage imageNamed:@"choiced_line.png"];
				cell.backgroundColor = [UIColor whiteColor];
				[cell addSubview:backImgView];
				
				if(![cell.selectedBackgroundView viewWithTag:10001])
				{
					UIImageView *backImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0, 8,46)];
					backImgView.image = [UIImage imageNamed:@"choiced_line.png"];
					backImgView.tag = 10001;
					[cell.selectedBackgroundView addSubview:backImgView];
				}
...
//				cell.selectedBackgroundView.backgroundColor = [UIColor redColor];
				if(![cell.selectedBackgroundView viewWithTag:10001])
				{
					UIImageView *backImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0, 10,46)];
					backImgView.image = [UIImage imageNamed:@"choiced_line.png"];
					backImgView.tag = 10001;
					[cell.selectedBackgroundView addSubview:backImgView];
				}
